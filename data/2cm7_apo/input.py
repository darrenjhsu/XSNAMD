...
data_path = 'data/2cm7_apo/'
fpsf = data_path + '2cm7_dry.psf'
fpdb = data_path + '2cm7_dry.pdb'
...
def next_n(x, n):
    ...
    #print(int(((int(x)+2047)/2048)*2048))
    print(int(np.ceil(float(x/n))*n))
...
for x in PSF:
    if get_bonds_from_now_on:
        ...
        temp_bonds = list(map(int, temp_bonds))
        while (idx < NBOND):
...
if S_exp_file is not None:
    ...
    with open(data_path + 'scat_param.cu','w') as f:
        f.write('#include \"scat_param.hh\"\n\n')
        f.write('int num_q = {:d};\n'.format(num_q))
        f.write('int num_q2 = {:d};\n'.format(next_n(num_q, 32)))
        f.write('float c1 = {:f};\n'.format(1.0))
        f.write('float c2 = {:f};\n'.format(2.0))
        f.write('float c = {:f};\n'.format(0.0))
        f.write('float q_S_ref_dS[{:d}] = {{'.format(num_q*3))
        f.write(', '.join(map(str,np.concatenate([q, q*0, q*0]))))
        f.write('};\n')
        f.write('float dS_err[{:d}] = {{'.format(num_q))
        f.write(', '.join(map(str,q*0)))
        f.write('};\n')
    with open(data_path + 'scat_param.hh','w') as f:
        f.write('extern int num_q;\n')
        f.write('extern int num_q2;\n')
        f.write('extern float c1;\n')
        f.write('extern float c2;\n')
        f.write('extern float c;\n')
        f.write('extern float q_S_ref_dS[{:d}];\n'.format(num_q*3))
        f.write('extern float dS_err[{:d}];\n'.format(num_q))
